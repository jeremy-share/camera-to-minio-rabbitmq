...
from image_analyzer.helpers import init_lib, DETECTION, DETNUMPAIR, METADATA, IMAGE, read_alt_names, DarkNetPredictionResult
...
class Detector:
    ...
    def __init__(
        ...
        # batch size = 1
        logger.info("Darknet::load_net_custom")
        self.net_main = self.load_net_custom(self.config_path.encode('ascii'), self.weights_path.encode('ascii'), 0, batch_size)
        logger.info("")

        logger.info("Darknet::load_meta")
        self.meta_main = self.load_meta(self.meta_path.encode('ascii'))
        logger.info("")

        # In Python 3, the metafile default access craps out on Windows (but not Linux)
        # Read the names file and create a list to feed to detect
        logger.info("Darknet::read_alt_names")
        self.alt_names = read_alt_names(self.meta_path)
        logger.info("")

        logger.info("Darknet::make_image")
